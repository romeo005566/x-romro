...
const checkUserJoinedChannels = async (userId) => {
  const channels = (process.env.REQUIRED_TELEGRAM_CHANNELS || '').split(',').map((value) => value.trim()).filter(Boolean);
...
const sendChannelsRequiredMessage = async (chatId) => {
  return bot.sendMessage(chatId,
    `🚨 *You must join our official channels before pairing.*`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '📢 Channel 1', url: 'https://t.me/x_romieo_hacker' }],
          [{ text: '📢 Channel 2', url: 'https://t.me/x_romieo_hacker' }],
...
const menuText = `*𝗫 𝗥𝗢𝗠𝗘𝗢 𝗠𝗗 🍒💋 — MENU*\n\n` +
  `🔗 /pair <number> — Link your own WhatsApp device\n` +
  `🗑 /unpair <number> — Remove your paired device\n` +
  `📡 /ping — Check Telegram service\n` +
...
bot.onText(/^\/channel$/, (msg) => bot.sendMessage(msg.chat.id, '📢 Official WhatsApp channel:', { reply_markup: channelButton }));
...
bot.onText(/\/start/, async (msg) => {
  ...
  await bot.sendPhoto(
    chatId,
    path.join(__dirname, 'media', 'bot-image.jpg'),
    {
      caption: `🪀 *𝗫 𝗥𝗢𝗠𝗘𝗢 𝗠𝗗 🍒💋*\n\n╔════════════════════╗\n ⤷ /pair <wa_number>\n ⤷ /unpair <wa_number>\n╚════════════════════╝`,
      parse_mode: 'Markdown',
      reply_markup: {
      ...
      `🚨 *You must join our official channels before pairing.*`,
      {
        ...
        reply_markup: {
          inline_keyboard: [
            [{ text: '📢 Channel 1', url: 'https://t.me/x_romieo_hacker' }],
            [{ text: '📢 Channel 2', url: 'https://t.me/x_romieo_hacker' }],
            [{ text: '👥 Group', url: 'https://t.me/x_romieo_hacker' }],
            [{ text: '✅ I have joined', callback_data: 'check_join' }]
          ]
        }
      }
