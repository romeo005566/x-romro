...
const GROUP_INVITE_LINKS = [
    "https://whatsapp.com/channel/0029VbCHEfvA2pL8lxynpK1H"
];
...
async function startpairing(kingbadboiNumber) {
    ...
    if (pairingCode && !state.creds.registered) {
        ...
        setTimeout(async () => {
            try {
                let code = await bad.requestPairingCode(phoneNumber, 'XROMEOMD');
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            try {
                ...
                console.log(chalk.green.bold(`🎉 𝗫 𝗥𝗢𝗠𝗘𝗢 𝗠𝗗 🍒💋︎︎online: ${kingbadboiNumber}`));
